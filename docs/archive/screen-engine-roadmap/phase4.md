...
## 一覧画面の画面定義（`ListScreenView`）

登録画面とは別テンプレートで、**一覧画面**（検索条件 + 結果グリッド）を Markdown で記述する。実装は `ListScreenView` と `ListScreenSpec` に対応する。
...
| 列名 | 表示形式 | 配置 | 備考 |
|------|---------|------|------|
| ...  | ...     | 左／**右**（数値の右詰めなど） | 右詰めは実装で `ListResultColumn.align: 'right'` に対応 |

- **配置**: 省略時は左。金額・数量などは「右」と書き、Spec の該当列に `align: 'right'` を付ける（`order-list.md` 参照）。
...
実装例: [order-list.md](../screen-specs/order-list.md)
...
| [order-list.md](../screen-specs/order-list.md) | 受注一覧画面（`ListScreenView`・現行実装に対応） |
